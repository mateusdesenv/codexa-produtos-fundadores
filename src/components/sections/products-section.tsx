    ...
    <section id="produtos" className="site-section scroll-mt-20" aria-labelledby="products-title">
      <div className="section-shell">
              ...
              <>
                ...
                <span className="relative z-10 flex min-h-[390px] flex-col p-6 sm:min-h-[430px] sm:p-8">
                  <span className="flex items-start justify-between gap-4">
                    ...
                    <span className="rounded-full border border-white/12 bg-[#071019]/65 px-3 py-1.5 text-[.68rem] font-black uppercase tracking-[.13em] text-slate-300 backdrop-blur-md">
                      {product.comingSoon ? "Em breve" : product.category}
                    </span>
                  </span>
                  <span className="mt-auto block text-2xl font-[850] tracking-[-.04em] text-white sm:text-3xl">{product.name}</span>
                  <span className="mt-3 block max-w-md text-sm leading-6 text-slate-300 sm:text-base">{product.tagline}</span>
                  <span className="mt-6 inline-flex items-center gap-2 text-sm font-extrabold text-green-strong">
        ...
        <article className="surface-card mt-5 flex flex-col items-start gap-5 rounded-[1.5rem] p-6 sm:flex-row sm:items-center sm:p-8">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-[.16em] text-green-strong">Em laboratório</p>
            <h3 className="mt-1.5 text-xl font-[850] text-white">Iris</h3>
          ...
          <span className="mt-1 rounded-full border border-white/10 px-4 py-2 text-xs font-black uppercase tracking-wider text-slate-400 sm:ml-auto sm:mt-0">Em evolução</span>
        </article>
      </div>
    </section>
  );
